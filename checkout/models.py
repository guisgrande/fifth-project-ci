...
class Order(models.Model):
    """
    Class model to contain all relevant informations about the order
    Like the user name and delivery adress details
    Order number and date of the order
    Oder total cost, delivery cost and final total
    """
    ...
    discount_total = models.DecimalField(max_digits=6, decimal_places=2,
...
class OrderLineItem(models.Model):
    """
    Class model to split order products in details
    """
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product.offer:
            self.lineitem_total = ((self.product.price) -
                                   ((self.product.price *
                                    self.product.offer.offer_discount)
                                    / 100)) * self.quantity
        else:
